...
for(i in 1:length(city_code$code)){
  for(j in types){
    ...
    #print(scheme)
    temp = Quandl(scheme) %>% 
      mutate(City=city_code$Region[i],County = city_code$County[i], Metro = city_code$Metro[i], Type = j)
